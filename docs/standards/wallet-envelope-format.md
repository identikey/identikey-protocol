...
**Key caching:** The derived 32-byte key is cached in the OS keychain (macOS Keychain, Linux Secret Service, Windows Credential Manager) keyed by `wallet-key-{blake3(wallet_path)[..16]}`. See [credential.rs](../../crates/identikey-wallet/src/credential.rs) for the `CredentialProvider` trait.
...
- [Wire Protocol](https://github.com/identikey/recrypt/blob/main/docs/wire-protocol.md) — conventions this format follows
- [dCBOR Determinism](dcbor-determinism.md) — interop contract for identity envelope serialization
- [Encoding Conventions](https://github.com/identikey/recrypt/blob/main/docs/standards/encoding-conventions.md) — normative rules for raw bytes / base58 / base64 / hex
- [Security Tiers](https://github.com/identikey/recrypt/blob/main/docs/security-tiers.md) — keyspace membership and capability extensions
- [Identity Self-Signature](https://github.com/identikey/recrypt/blob/main/docs/standards/identity-self-signature.md) — spec for `sign_self_ed25519` / `verify_self_signature_ed25519`
