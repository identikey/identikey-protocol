...
//! environments and testing. The keychain service name and environment
//! variable names come from the application's [`WalletParams`].

use anyhow::{anyhow, Result};
use base64::Engine;
use std::sync::RwLock;

use super::format::WalletParams;
...
impl MacOSKeychainProvider {
    pub fn new(wallet_path: &std::path::Path, service_name: &'static str) -> Self {
        Self {
            service_name,
    ...
    pub fn legacy(service_name: &'static str) -> Self {
        Self {
            service_name,
...
///
/// Reads the app's `env_key` variable (see [`WalletParams`]) as a
/// base64-encoded 32-byte key. Cannot store or clear keys (env vars are
...
/// get separate cached keys and don't stomp each other. Service and env var
/// names come from the application's [`WalletParams`].
///
/// Priority:
/// 1. `env_no_keychain` set → in-memory provider (testing/CI without keychain)
/// 2. `env_key` set → env provider (CI mode)
/// 3. OS keyring (interactive use, wallet-specific entry)
/// 4. Memory fallback (per-process only, for headless systems without keyring)
pub fn default_provider_for(
    wallet_path: &std::path::Path,
    params: &'static WalletParams,
) -> Box<dyn CredentialProvider> {
    // 0. Check for no-keychain mode (testing/CI without keychain access)
    if std::env::var(params.env_no_keychain).is_ok() {
        return Box::new(MemoryProvider::new());
    }

    // 1. Check for CI mode via env var
    let env = EnvProvider::new(params.env_key);
    if env.is_available() {
        return Box::new(env);
    }
    ...
    #[cfg(target_os = "windows")]
    {
        Box::new(KeyringProvider::new(wallet_path, params.keychain_service))
    }

    // 4. Linux: check if Secret Service is available; fall back to memory if not
    #[cfg(target_os = "linux")]
    {
        let keyring = KeyringProvider::new(wallet_path, params.keychain_service);
...
pub fn legacy_provider(params: &'static WalletParams) -> Box<dyn CredentialProvider> {
    #[cfg(target_os = "macos")]
    {
        Box::new(MacOSKeychainProvider::legacy(params.keychain_service))
    }

    #[cfg(not(target_os = "macos"))]
    {
        Box::new(KeyringProvider::legacy(params.keychain_service))
    }
}
...
mod tests {
    ...
    fn test_env_provider_with_valid_key() {
        ...
        unsafe { std::env::set_var("TEST_IKEY_WALLET_KEY_VALID", &encoded) };

        let provider = EnvProvider::new("TEST_IKEY_WALLET_KEY_VALID");
        assert!(provider.is_available());
        assert_eq!(provider.get_key().unwrap(), Some(key));

        unsafe { std::env::remove_var("TEST_IKEY_WALLET_KEY_VALID") };
    }
    ...
    fn test_env_provider_invalid_length() {
        ...
        unsafe { std::env::set_var("TEST_IKEY_WALLET_KEY_SHORT", &encoded) };

        let provider = EnvProvider::new("TEST_IKEY_WALLET_KEY_SHORT");
        let result = provider.get_key();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("32 bytes"));

        unsafe { std::env::remove_var("TEST_IKEY_WALLET_KEY_SHORT") };
    }
}
